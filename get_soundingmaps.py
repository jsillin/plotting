...
if season == 'winter':
    ...
    for i in range(6,etime):
        ...
        ref_levs = [10,15,20, 25, 30, 35, 40, 45, 50, 55, 60, 65]
        qr_cols = ['#85ff5c','#60ff2b','#40ff00','#ffff00','#e6e600','#cccc00','#e4cc00','#ff9933','#ff6600','#ff5050','#ff0066']
        ...
        spt.addwintercapecolorbar(ax1,fig,capep,capelevs)
        spt.addwinterrefcolorbar(ax1,fig,ra,ref_levs)
        try:
            spt.addsnowcolorbar(ax1,fig,sn,ref_levs)
        except:
            print('no snow')
        try:
            spt.addsleetcolorbar(ax1,fig,ip,ref_levs)
        except:
            print('no sleet')
        try:
            spt.addicecolorbar(ax1,fig,zr,ref_levs)
        except:
            print('no ice')
    ...
    for i in range(6,etime):
        ...
        #Save plot
        plt.savefig(output_dir+'/'+model+'/'+domainname+'_severe_soundingmap_'+dtfs+'.png',bbox_inches='tight',pad_inches=0.1)
        plt.close()
        plt.clf()
