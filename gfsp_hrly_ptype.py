...
for i in range(2,120):
    ...
    plt.savefig(output_dir+'/GFS_para/ne_hrly_pytpe_'+dtfs+'.png')
    ...
    fig2.tight_layout()
    plt.savefig(output_dir+'/GFS_para/ec_accum_ptype_'+dtfs+'.png')
    ...
    plt.savefig(output_dir+'/GFS_para/ne_accum_ptype_'+dtfs+'.png')
    plt.close()
    plt.clf()
    ...
    ax6.set_title('GFS (v16) Ice Accretion Forecast (FRAM)',fontsize=14)
    ax6.set_title(' \n Valid: '+time.dt.strftime('%Y-%m-%d %H:%MZ').item(),fontsize=11,loc='right')
    ax6.set_title(' \n Init: '+init_time.dt.strftime('%Y-%m-%d %H:%MZ').item(),fontsize=11,loc='left')
    ax6.set_extent((265, 300, 25, 50))#, crs = zH5_crs)    # Set a title and show the plot
    plt.savefig(output_dir+'/GFS_para/ec_ice_accretion_fram_'+dtfs+'.png')
    ax6.set_extent((281,295,39,49))
    plt.savefig(output_dir+'/GFS_para/ne_ice_accretion_fram_'+dtfs+'.png')
    ax6.set_extent((280,282,36,40))
    plt.savefig(output_dir+'/GFS_para/wv_ice_accretion_fram_'+dtfs+'.png')
    plt.close()
    plt.clf()
