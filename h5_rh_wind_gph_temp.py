...
for i in range(0,41):
    ...
    ax.set_title('500 hPa Heights (m), Temperature (\u00B0C), Humidity (%) at ' + time[0].dt.strftime('%Y-%m-%d %H:%MZ').item())
    plt.savefig(output_dir+'/H5/height_temp_RH_'+time[0].dt.strftime('%Y-%m-%d %H%M').item()+'.png')
    fcst_hr = str(i*3)
