...
for i in range(1,120):
    ...
    u8 = data['u'].sel(lev=850.0,lat=lats,lon=lons).squeeze()*1.94384449
    v8 = data['v'].sel(lev=850.0,lat=lats,lon=lons).squeeze()*1.94384449
    h8 = data['gph'].sel(lev=850.0,lat=lats,lon=lons).squeeze()
    pwat = data['pwat'].sel(lat=lats,lon=lons).squeeze()*0.0393700787402
    x, y = h8.metpy.coordinates('x', 'y')
    ...
    ########## PLOTTING #######################################################
    h8c = ax1.contour(x,y,h8,colors='dimgray', levels = range(1100,1600,30),linewidths=1.5)
    #t2c = ax1.contour(x,y,t2,colors='r', levels = range(-60,0,5),linestyles='dashed',linewidths=1)
    pwc = ax1.contourf(x,y,pwat,cmap='Greens',levels=pwlevs,alpha=0.9)
    a5cb = fig.colorbar(pwc, orientation = 'horizontal', aspect = 80, ax = ax1, pad = 0.01,
                        extendrect=False, ticks = [0.5,1,1.5,2,2.5])
    a5cb.set_label('Precipitable Water (in)', fontsize = 12)
    ...
    ax1.set_title('Precipitable Water (in), 850mb Heights (m), and Winds (kts)',fontsize=16)
    ...
    plt.savefig(output_dir+'/GFS/gfs_hrly_pwat_v2_'+str(i)+'.png')
    fcst_hr = str(0)
