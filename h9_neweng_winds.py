...
for i in range(0,41):
    ...
    ncss = best_ds.subset()
    query = ncss.query()
    query.lonlat_box(north=47, south=40, east=-65, west=-75).time(datetime.utcnow()+dt.timedelta(hours=i*3))
    query.accept('netcdf4')
    ...
    wind_contour = ax.contour(x, y, sped_H9, colors='xkcd:blue', levels=range(30, 100, 10), alpha=0.9, linestyles='-')
    ...
    plt.savefig(output_dir+'/H9/newng_winds'+time[0].dt.strftime('%Y-%m-%d %H%M').item()+'.png')
    fcst_hr = str(i*3)
