...
namdate = spt.get_init_time('NAM')[0]
gfsdate = spt.get_init_time('GFS')[0]
nam_hour = spt.get_init_time('NAM')[1]
gfs_hour = spt.get_init_time('GFS')[1]

nam_url='http://nomads.ncep.noaa.gov:80/dods/nam/nam'+namdate+'/nam_'+nam_hour+'z'
gfs_url = 'http://nomads.ncep.noaa.gov:80/dods/gfs_0p25_1hr/gfs'+gfsdate+'/gfs_0p25_1hr_'+gfs_hour+'z'

# Create new directory
output_dir = namdate+'_'+nam_hour+'00'
#output_dir = '20201231_0000'
spt.mkdir_p(output_dir)
spt.mkdir_p(output_dir+'/comparison')

print(nam_url)
print(gfs_url)

nam_ds = xr.open_dataset(nam_url)
nam_times = nam_ds['tmp2m'].metpy.time
gfs_ds = xr.open_dataset(gfs_url)
gfs_times = gfs_ds['tmp2m'].metpy.time

nam_lats = np.arange(12.21990800000,61.20556254545,0.110827275)
nam_lons = np.arange((360-152.87862300000),(360.-49.47263081081),0.11338376)

gfs_lats = np.arange(25,55,0.25)
gfs_lons = np.arange(260,310,0.25)
init_time = nam_ds['time'][0]
ginit_time = gfs_ds['time'][0]
#Now loop through the rest to come up with a ptype map each hour and
#ideally a four-panel accumulation map
for i in range(0,28):
    nam_data = nam_ds.metpy.parse_cf()
    nam_data = nam_data.isel(time=i)
    gfs_data = gfs_ds.metpy.parse_cf()
    gfs_data = gfs_data.sel(time=nam_data.time.values)


    #Rename variables to useful things
    nam_data = nam_data.rename({
        'cfrzrsfc':'catice',
        'cicepsfc':'catsleet',
        'crainsfc':'catrain',
        'csnowsfc':'catsnow',
    ...
    catrain = nam_data['catrain'].squeeze()
    catsnow = nam_data['catsnow'].squeeze()
    catsleet = nam_data['catsleet'].squeeze()
    catice = nam_data['catice'].squeeze()
    ...
    #Coordinate stuff
    vertical, = nam_data['temperature'].metpy.coordinates('vertical')
    time = nam_data['temperature'].metpy.time
    x, y = nam_data['temperature'].metpy.coordinates('x', 'y')
    gx,gy = gfs_data['temperature'].metpy.coordinates('x', 'y')
    lat, lon = xr.broadcast(y, x)
    zH5_crs = nam_data['temperature'].metpy.cartopy_crs

    #data['temperature'].metpy.convert_units('degC')
    nam_t2m = nam_data['sfc_temp'].squeeze()
    nam_t2m = ((nam_t2m - 273.15)*(9./5.))+32.

    gfs_t2m = gfs_data['sfc_temp'].squeeze()
    gfs_t2m = ((gfs_t2m - 273.15)*(9./5.))+32.

    reflectivity = nam_data['radar'].squeeze()
    ...
    ntmp_2m32 = ax1.contour(x,y,nam_t2m,colors='b', alpha = 0.8, levels = [32])
    gtmp_2m32 = ax1.contour(gx,gy,gfs_t2m,colors='steelblue', alpha = 0.8, levels = [32])
    ...
    ax1.set_title('NAM Precipitation Type, MSLP, and Selected NAM/GFS Soundings',fontsize=14)
    ax1.set_title('\n Valid: '+time.dt.strftime('%Y-%m-%d %H:%MZ').item(),fontsize=10,loc='right')
    ax1.set_title('\n NAM Init: '+init_time.dt.strftime('%Y-%m-%d %H:%MZ').item()+'\n GFS Init: '+ginit_time.dt.strftime('%Y-%m-%d %H:%MZ').item(),fontsize=10,loc='left')
    ax1.set_extent((279, 294, 36, 48))#, crs = zH5_crs)    # Set a title and show the plot

    soundlat = 36
    soundlon = 360-99

    nam_sound_pres = nam_data.lev
    gfs_sound_pres = gfs_data.lev
    ptop=300
    startlon=67.6
    londelt=2.9
    sound_lons = []
    ...
    #sound_lats = [29.6,31.5,33.5,35.4,37.4,39.25]
    sound_lats=[soundlat+0.6,soundlat+2.5,soundlat+4.5,soundlat+6.4,soundlat+8.4,soundlat+10.25]
    for i in range(1,r):
        skew = SkewT(fig=fig,rect=(0.75-(0.15*i),0.2,.15,.1))

        soundlat = sound_lats[0]
        soundlon = -(startlon+(londelt*i))
        gsoundlon = 360+soundlon
        nam_sound_temps = nam_data['temperature'].interp(lat=soundlat,lon=soundlon)-273.15
        gfs_sound_temps = gfs_data['temperature'].interp(lat=soundlat,lon=gsoundlon)-273.15
        nam_sound_rh = nam_data['rh'].interp(lat=soundlat,lon=soundlon)
        gfs_sound_rh = gfs_data['rh'].interp(lat=soundlat,lon=gsoundlon)
        nam_sound_dp = mpcalc.dewpoint_from_relative_humidity(nam_sound_temps.data*units.degC,nam_sound_rh.data*units.percent)
        gfs_sound_dp = mpcalc.dewpoint_from_relative_humidity(gfs_sound_temps.data*units.degC,gfs_sound_rh.data*units.percent)

        skew.plot(nam_sound_pres,nam_sound_dp,'g',linewidth=2)
        skew.plot(gfs_sound_pres,gfs_sound_dp,'limegreen',linewidth=2)
        skew.plot(nam_sound_pres,nam_sound_temps,'r',linewidth=2)
        skew.plot(gfs_sound_pres,gfs_sound_temps,'indianred',linewidth=2)
    ...
    for i in range(0,r):
        ...
        #print(nam_sound_temps)
        print(gfs_sound_temps)
    ...
    for i in range(0,r):
        ...
        nam_sound_dp = mpcalc.dewpoint_from_relative_humidity(nam_sound_temps.data*units.degC,nam_sound_rh.data*units.percent)
        gfs_sound_dp = mpcalc.dewpoint_from_relative_humidity(gfs_sound_temps.data*units.degC,gfs_sound_rh.data*units.percent)
        modeldiff = nam_sound_temps-gfs_sound_temps
        print(modeldiff)
        print(nam_sound_temps)
        print(gfs_sound_temps)
        skew.plot(nam_sound_pres,nam_sound_dp,'g',linewidth=2)
    ...
    dashed_red_line = lines.Line2D([], [], linestyle='solid', color='r', label='NAM Temperature')
    dashed_gred_line = lines.Line2D([], [], linestyle='solid', color='indianred', label='GFS Temperature')

    dashed_purple_line = lines.Line2D([],[],linestyle='dashed',color='purple',label='0C Isotherm')
    dashed_green_line = lines.Line2D([], [], linestyle='solid', color='g', label='NAM Dew Point')
    dashed_ggreen_line = lines.Line2D([], [], linestyle='solid', color='limegreen', label='GFS Dew Point')

    grey_line = lines.Line2D([], [], color='darkgray', label='MSLP (hPa)')
    blue_line = lines.Line2D([], [], color='b',label='2m 0C Isotherm')
    leg = ax1.legend(handles=[dashed_red_line,dashed_gred_line,dashed_green_line,dashed_ggreen_line,dashed_purple_line],title='Sounding Legend',loc=4,framealpha=1)
    leg.set_zorder(100)

    plt.savefig(output_dir+'/comparison/nam_vs_gfs_pytpe_sound_newengland_v4_'+dtfs+'.png',bbox_inches='tight',pad_inches=0.1)
    fcst_hr = str(0)
    plt.close()
    plt.clf()
    '''
