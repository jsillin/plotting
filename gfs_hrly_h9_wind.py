...
lats = np.arange(25,60,0.25)
lons = np.arange(260,330,0.25)
...
for i in range(60,78):
    ...
    t9 = data['temp'].sel(lev=925.0,lat=lats,lon=lons)-273.15
    u9 = data['u'].sel(lev=925.0,lat=lats,lon=lons).squeeze()*1.94384449
    v9 = data['v'].sel(lev=925.0,lat=lats,lon=lons).squeeze()*1.94384449
    h9 = data['gph'].sel(lev=925.0,lat=lats,lon=lons).squeeze()
    ts = data['t2m'].sel(lat=lats,lon=lons).squeeze()-273.15
    ws9 = ((u9**2)+(v9**2))**.5
    x, y = t9.metpy.coordinates('x', 'y')
    lat, lon = xr.broadcast(y, x)
    wind_slice = slice(1,-1,1)
    lr = ((ts-t9)/h9)*1000
    wind_est = (0.5*ws9)+(0.9*lr)-(0.05*h9)+31
    pwm=wind_est.sel(lat=43.645,lon=360-70.30525,method='nearest')
    pwm_est.append(pwm.values)
    ...
    ########## PLOTTING #######################################################
    h9c = ax1.contour(x,y,h9,colors='dimgray', levels = range(450,990,30),linewidths=1.5)
    t9c = ax1.contour(x,y,t9,colors='xkcd:light red', levels = range(-60,40,5),linestyles='dashed',linewidths=1)
    lr_levs = np.arange(-15,15,3)
    lr_cf = ax1.contourf(lon, lat, lr, lr_levs, alpha = 0.7, cmap=plt.cm.RdBu)
    lcb = fig.colorbar(lr_cf, orientation = 'horizontal', aspect = 80, ax = ax1, pad = 0.01,
                        extendrect=False, ticks = range(-15,18,3))
    lcb.set_label('Surface-925mb Lapse Rate (C/km)')
    ax1.barbs(x[wind_slice],y[wind_slice],u9[wind_slice,wind_slice],v9[wind_slice,wind_slice], length=7)

    clevs_900_sped = np.arange(30, 110, 10)
    wind_contour = ax1.contour(x, y, ws9, colors='xkcd:blue', levels=range(30, 100, 10), alpha=0.9, linestyles='-')
    wind_contour.clabel(fontsize=14, colors='b', inline=1, inline_spacing=2, fmt='%i', rightside_up=True, use_clabeltext=True)
    ...
    ax1.set_title('925mb Height (m), Wind (kts), & Sfc-925mb Lapse Rate (C/km))',fontsize=16)
    ...
    plt.savefig(output_dir+'/GFS/gfs_hrly_h9wind_ec_'+dtfs+'.png')
    ax1.set_extent((287,292,42,46))
    plt.savefig(output_dir+'/GFS/gfs_hrly_h9wind_menh_'+dtfs+'.png')
    plt.clf()
    plt.close()
    ...
    a5c2 = ax2.contourf(x,y,wind_est,cmap='autumn_r',levels=range(0,70,5),alpha=0.8)
    a5cb2 = fig2.colorbar(a5c2, orientation = 'horizontal', aspect = 80, ax = ax2, pad = 0.01,
                        extendrect=False, ticks = range(0,75,10))
    a5cb2.set_label('Estimated Surface Wind Gust', fontsize = 12)

    #h_contour = ax1.contour(x, y, mslpc, colors='dimgray', levels=range(940,1040,4),linewidths=2)
    #h_contour.clabel(fontsize=14, colors='dimgray', inline=1, inline_spacing=4, fmt='%i mb', rightside_up=True, use_clabeltext=True)
    ax2.set_title('Estimated Surface Wind Gust',fontsize=16)
    ax2.set_title('\n Valid: '+time.dt.strftime('%Y-%m-%d %H:%MZ').item(),fontsize=11,loc='right')
    ax2.set_title('\n GFS Init: '+init_time.dt.strftime('%Y-%m-%d %H:%MZ').item(),fontsize=11,loc='left')
    ax2.set_extent((287,292,42,46))
    plt.savefig(output_dir+'/GFS/gfs_hrly_h9_windest_v1_'+dtfs+'.png')
    plt.clf()
    plt.close()
...
a5cb2.set_label('925mb Absolute Vorticity ($s^{-1}$)', fontsize = 12)
ax3.barbs(x[wind_slice_s],y[wind_slice_s],u5[wind_slice_s,wind_slice_s],v5[wind_slice_s,wind_slice_s], length=7)

#h_contour = ax1.contour(x, y, mslpc, colors='dimgray', levels=range(940,1040,4),linewidths=2)
#h_contour.clabel(fontsize=14, colors='dimgray', inline=1, inline_spacing=4, fmt='%i mb', rightside_up=True, use_clabeltext=True)
ax3.set_title('925mb Heights (m) and Absolute Vorticity ($s^{-1}$)',fontsize=16)
ax3.set_title('\n Valid: '+time.dt.strftime('%Y-%m-%d %H:%MZ').item(),fontsize=11,loc='right')
ax3.set_title('\n GFS Init: '+init_time.dt.strftime('%Y-%m-%d %H:%MZ').item(),fontsize=11,loc='left')
