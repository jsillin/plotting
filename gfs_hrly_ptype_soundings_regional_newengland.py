...
def Td(t_FHAG2, rh_FHAG2):
    sat_pressure_0c = 6.112
    svp = sat_pressure_0c * exp(17.67 * (t_FHAG2 - 273.15) / (t_FHAG2 - 29.65))
    e = (rh_FHAG2 / 100.) * svp
...
#url = 'http://nomads.ncep.noaa.gov:80/dods/gfs_0p25_1hr/gfs'+mdate+'/gfs_0p25_1hr_06z'
...
for i in range(0,120):
    ...
    for i in range(0,r):
        ...
        skew.ax.axvline(0, color='purple', linestyle='--', linewidth=3)
        skew.ax.set_ylim((1000,ptop))
        skew.ax.axis('off')        #skew.ax.text(0.1,0.1,str(soundlat)+'  '+str(soundlon))
    ...
    plt.savefig(output_dir+'/GFS/gfs_hrly_pytpe_sound_newengland_v2_'+dtfs+'.png',bbox_inches='tight',pad_inches=0.1)
    fcst_hr = str(0)
    plt.close()
    plt.clf()
    '''
