...
def get_init_hr(hour):
    if int(hour) <8:
        init_hour = '0'+str(startTime.hour+2)
    else:
        init_hour = str(startTime.hour+2)
    return(init_hour)

def get_forecast_hour(hr):
    if int(hr) <10:
        forecast_hour = '00'+str(hr)
    elif int(hr) <100:
        forecast_hour = '0'+str(hr)
    else:
        forecast_hour = str(hr)
    return forecast_hour

init_hour = get_init_hr(hour)
init_hr=init_hour
#url='http://nomads.ncep.noaa.gov:80/dods/blend/blend'+mdate+'/blend_6hr_'+init_hour+'z'

# Create new directory
output_dir = str(year)+str(month)+str(day)+'_'+str(init_hour)+'00'
mkdir_p(output_dir)
mkdir_p(output_dir+'/NBM')


#total_precip=ds['apcpsfc'].squeeze()*.0393700787402

for i in range(0,36):
    forecast_hour = get_forecast_hour(i)
    url ='ftp://ftp.ncep.noaa.gov/pub/data/nccf/com/blend/prod/blend.'+mdate+'/'+init_hour+'/core/blend.t'+init_hour+'z.core.f'+forecast_hour+'.co.grib2'
    ds = xr.open_dataset('https://nomads.ncep.noaa.gov/pub/data/nccf/com/blend/prod/blend.20201230/10/core/blend.t10z.core.f028.co.grib2',engine='cfgrib')
    print(ds)
    '''
    ...
    time = data['qpf'].metpy.time
    qpf = data['qpf'].squeeze()*.0393700787402
    print(np.max(data['qpf']))
    ra = data['ra_ts'].squeeze()
    zr = data['zr_mx'].squeeze()
    ip = data['ip_gp'].squeeze()
    sn = data['sn_mx'].squeeze()
    print(sn)
    print(np.max(sn))
    ...
    try:
        probc = ax.contourf(x,y,qpf,levels=np.linspace(0.05,10,200),cmap='gist_stern',alpha=0.8)
        cbar = fig.colorbar(probc, orientation='horizontal',aspect=80,ax=ax,pad=0.01,extendrect=False,ticks=np.linspace(0,10,40))
    except:
        print('not enough snow :(')
    ax.set_title('QPF',fontsize=14)
    ax.set_title('Valid: '+time.dt.strftime('%a %b %d %H:%MZ').item(),fontsize=11,loc='right')
    ax.set_title('NBM Init: '+init_time.dt.strftime('%Y-%m-%d %H:%MZ').item(),fontsize=11,loc='left')
    plt.savefig(output_dir+'/NBM/us_qpf_'+dtfs+'.png')
    ax.set_extent((260, 295, 25, 50))#, crs = zH5_crs)    # Set a title and show the plot
    plt.savefig(output_dir+'/NBM/ec_qpf_'+dtfs+'.png')
    ax.set_extent((281,295,39,49))#, crs = zH5_crs)    # Set a title and show the plot
    plt.savefig(output_dir+'/NBM/ne__qpf_'+dtfs+'.png')
    ax.set_extent((289,291,43,45))#, crs = zH5_crs)    # Set a title and show the plot
    plt.savefig(output_dir+'/NBM/me__qpf_'+dtfs+'.png')
    plt.close()
    plt.clf()
    '''
