...
init_hour = startTime.hour+2
init_hr=init_hour
url='http://nomads.ncep.noaa.gov:80/dods/blend/blend'+mdate+'/blend_6hr_12z'
...
for i in range(0,36):
    ...
    time = data['ps01'].metpy.time
    print(np.max(data['ps01']))
    p01 = data['ps01'].squeeze()
    p2 = data['ps2'].squeeze()
    p4 = data['ps4'].squeeze()
    p6 = data['ps6'].squeeze()
    p8 = data['ps8'].squeeze()
    p12 = data['ps12'].squeeze()
    p18 = data['ps18'].squeeze()
    p24 = data['ps24'].squeeze()
    p30 = data['ps30'].squeeze()
    print(np.max(p01))
    probs = [p01,p2,p4,p6,p8,p12,p18,p24,p30]
    inches = [0.1,2,4,6,8,12,18,24,30]
    x, y = p2.metpy.coordinates('x', 'y')
    ...
    for j in range(len(probs)):
        ...
        try:
            probc = ax.contourf(x,y,probs[j],levels=np.linspace(0.05,1.0,18),cmap='YlGnBu',alpha=0.8)
            cbar = fig.colorbar(probc, orientation='horizontal',aspect=80,ax=ax,pad=0.01,extendrect=False,ticks=np.linspace(0,1.0,10))
        except:
            print('not enough snow :(')
        ax.set_title('Probability of Snowfall Exceeding '+str(inches[j])+'"',fontsize=14)
        ...
        plt.savefig(output_dir+'/NBM/us_'+str(inches[j])+'inch_snow_probs'+dtfs+'.png')
        ax.set_extent((260, 295, 25, 50))#, crs = zH5_crs)    # Set a title and show the plot
        plt.savefig(output_dir+'/NBM/ec_'+str(inches[j])+'inch_snow_probs'+dtfs+'.png')
        ax.set_extent((281,295,39,49))#, crs = zH5_crs)    # Set a title and show the plot
        plt.savefig(output_dir+'/NBM/ne_'+str(inches[j])+'inch_snow_probs'+dtfs+'.png')
        ax.set_extent((289,291,43,45))#, crs = zH5_crs)    # Set a title and show the plot
        plt.savefig(output_dir+'/NBM/me_'+str(inches[j])+'inch_snow_probs'+dtfs+'.png')
        plt.close()
        plt.clf()
    '''
