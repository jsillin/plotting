        ...
        if wetbulb==True:
            ...
            for k in range(len(plot_elevs)):
                ...
                for i in range(s,r):
                    ...
                    #print(spt.mask_below_terrain(spres,sound_temps,sound_pres))
                          
                    #Calculate virtual temperature
                    print(abv_sfc_temp[0])
                    print(abv_sfc_relh[0])
                    #sfc_mix_ratio = mpcalc.mixing_ratio_from_relative_humidity(pres_abv_ground[0],abv_sfc_temp[0].data*units.degC,abv_sfc_relh[0].data*units.degC)
                    print(sfc_mix_ratio)
                    #surface_virtual_temperature = mpcalc.virtual_temperature(abv_sfc_temp[0].data*units.degC,sfc_mix_ratio)
                    print(surface_virtual_temperature)
                    
                    muparcel = mpcalc.most_unstable_parcel(pres_abv_ground,abv_ground_temp,abv_ground_dewp)[1]
                    
                    #sound_wb = sound_wb*units.degC
                    skew = SkewT(fig=fig,rect=(0.75-(0.15*i),plot_elev,.15,.1))

                    sbparcel_prof = mpcalc.parcel_profile(pres_abv_ground,abv_sfc_temp[0].data*units.degC,abv_sfc_dewp[0])
                    mlparcel_prof = mpcalc.mixed_parcel(pres_abv_ground,abv_sfc_temp,abv_sfc_dewp)
                    muparcel_prof = mpcalc.parcel_profile(pres_abv_ground,abv_sfc_temp[0])
                    sb_tv_parcel_prof = mpcalc.parcel_profile(pres_abv_ground,surface_virtual_temperature,abv_sfc_dewp[0])
                    
                    cape = mpcalc.cape_cin(pres_abv_ground,abv_sfc_temp.data*units.degC,abv_sfc_dewp,mlparcel_prof)
                    ...
                    if capeout >100:
                        ...
                        skew.plot(pres_abv_ground,sbparcel_prof,color='orchid',linewidth=1.5,linestyle='--')
                        skew.plot(pres_abv_ground,sb_tv_parcel_prof,color='fuchsia',linewidth=1.5,linestyle='--')
                        skew.plot(pres_abv_ground,muparcel_prof,color='orange',linewidth=1.5,linestyle='--')
                        skew.plot(pres_abv_ground,mlparcel_prof,color='orangered',linewidth=1.5)                                                              
