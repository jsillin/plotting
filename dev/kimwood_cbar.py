...
def addcolorbar(ax):
    ...
    left = axes_bbox.x0 + 0.01
    bottom = axes_bbox.y0 + 0.01
    width = 0.4
    height = 0.02
    cax = fig.add_axes([left, bottom, width, height])
    cbar = plt.colorbar(im, cax=cax, ticks=clevs, orientation='horizontal')
    cbar.ax.tick_params(labelsize=10)
    cbar.set_label('CAPE (J/kg)', size=8)  # MODIFY THIS for other fields!!


"""
Code begins below; modify 'ymdh' as needed for experimenting (since Unidata THREDDS only keeps a few days of forecasts)
"""
ymdh = '2021030600'  # YYYYMMDDHH; user can change to a command-line input with the line ymdh = sys.argv[1]
...
plt.savefig('testingcbar_v6_.png')
