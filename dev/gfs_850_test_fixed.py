...
for i in range(0,41):
    ...
    q_div = ndimage.gaussian_filter(q_div,sigma=5,order=0)
    ...
    #ax.quiver(lon[wind_slice[0]], lat[wind_slice[1]],
          #uqvect[wind_slice].m,
          #vqvect[wind_slice].m,
    ...
    plt.savefig(output_dir+'/H85/height_temp_qvec3_'+time[0].dt.strftime('%Y-%m-%d %H%M').item()+'.png')
    fcst_hr = str(i*3)
