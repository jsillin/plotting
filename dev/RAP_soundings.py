...
startTime=datetime.utcnow()-dt.timedelta(hours=1)
...
for i in range(0,36):
    ...
    capep = ax1.contourf(x, y, cape, levels=[25,50,75,100,150, 250, 500, 750, 1000], extend='max', alpha = 0.7, cmap='RdPu')#['#0099ff00', '#4066ffb3', '#8066ff8c', '#BF66ff66','#8cff66','#b3ff66','#d9ff66','#ffff66','#ffd966','#ffcc66','#ffb366','#ff8c66','#ff6666','#ff668c','#ff66b3','#ff66d9','#ff66ff'])
    ...
    ax1.set_extent((270.5, 274.5, 28.875, 32.125))#, crs = zH5_crs)    # Set a title and show the plot
    ...
    ######## Save the plot
    plt.savefig(output_dir+'/RAP/RAP_hrly_pytpe_hop_sound_'+dtfs+'.png',bbox_inches='tight',pad_inches=0.1)
    fcst_hr = str(0)
    plt.close()
    plt.clf()
