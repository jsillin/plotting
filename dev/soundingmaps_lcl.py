    ...
    if cape==True:
        for k in range(len(plot_elevs)):
            ...
            for i in range(s,r):
                ...
                lcl_pressure = mpcalc.lcl(sound_pres[2].values*units.hPa,sound_temps[2].values*units.degC,sound_dp[2])
                print(lcl_pressure)
                lcl_temperature = mpcalc.lcl(sound_pres[2].values*units.hPa,sound_temps[2].values*units.degC,sound_dp[2])
                lcl_hgt = np.round(mpcalc.pressure_to_height_std(lcl_pressure[0]), decimals=3).to(units.meter)
                #sound_wb = mpcalc.wet_bulb_temperature(sound_pres[2].values*units.hPa,sound_temps[2].values*units.degC,sound_dp[2])
