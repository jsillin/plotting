...
centerlat=input('Center Lat (Deg N): ')
centerlon=input('Center Lon (Deg W): ')
state_abbr=input('State Abbreviation: ')
...
for i in range(0,49):
    ...
    ax1.set_title('Precip Type, 10m Winds (kts), and MSLP (mb)',fontsize=14)
    ...
    sub_w = 360-(float(centerlon)-7.5)
    sub_e = 360-(float(centerlon)+7.5)
    sub_n = float(centerlat)+6
    sub_s = float(centerlat)-6

    ax1.set_extent((sub_w, sub_e, sub_s, sub_n))#, crs = zH5_crs)    # Set a title and show the plot
    ...
    #fig.canvas.draw()
    fig.tight_layout()
    plt.savefig(output_dir+'/HRRR_ex/'+state_abbr+'_svr_dash_v20_'+dtfs+'_.png',bbox_inches='tight',pad_inches=0.1)
