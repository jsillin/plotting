...
mkdir_p(output_dir)
mkdir_p(output_dir+'/NAM/TYS/Wind') #create subdirectory to store GFS output like this
...
for i in range(0,84):
    ...
    gust = data['gust'].squeeze()
    gust = gust*1.94384449

    wind_slice = slice(4, -4, 4)
    print(data['u_pres'])
    u_press = data['u_pres'].metpy.sel(lev=round(850,0)).squeeze()
    v_press = data['v_pres'].metpy.sel(lev=round(850,0)).squeeze()
    u_knots = u_pres*1.94384449
    v_knots = v_pres*1.94384449
    wspd_pres = ((u_knots**2)+(v_knots**2))**.5
    ...
    ax1.barbs(x[wind_slice],y[wind_slice],u_pres[wind_slice,wind_slice],v_pres[wind_slice,wind_slice], length=7)


    #Set a title and extent for the map
    ax1.set_title('Wind Gust, and Selected Soundings',fontsize=16)
    ...
    ######## Save the plot
    plt.savefig(output_dir+'/NAM/TYS/Wind/NAM_hrly_tys_sound_'+dtfs+'.png',bbox_inches='tight',pad_inches=0.1)
    fcst_hr = str(0)
    plt.close()
    plt.clf()
