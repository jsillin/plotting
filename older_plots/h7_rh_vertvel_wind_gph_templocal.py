...
ncss = best_ds.subset()
query = ncss.query()
query.lonlat_box(north=50, south=40, east=-65, west=-80).time(datetime.utcnow())
...
for i in range(0,41):
    ...
    ncss = best_ds.subset()
    query = ncss.query()
    query.lonlat_box(north=50, south=40, east=-65, west=-80).time(datetime.utcnow()+dt.timedelta(hours=3*i))
    query.accept('netcdf4')
    ...
    rh = ax.contourf(x, y, rel_hum, levels=[70, 80, 90, 100], alpha = 0.5, colors=['#99ff00', '#00ff00', '#00cc00'])

    vertvel = data['omega'].metpy.sel(vertical=700*units.hPa).squeeze()
    vv = ax.contourf(x,y,vertvel, levels = [-5,-4.5,-4,-3.5,-3,-2.5,-2,-1.5,-1,-.5], alpha = 0.5, colors = ['#ffe6f9', '#ffccf3', '#ffb3ed', '#ff99e7', '#ff80e1','#ff66db','#ff4dd5','#ff33cf','#ff1ac9','#00ffffff'])
    ...
    plt.savefig(output_dir+'/H7_local/height_temp_RH_local'+time[0].dt.strftime('%Y-%m-%d %H%M').item()+'.png')
    fcst_hr = str(i*3)
