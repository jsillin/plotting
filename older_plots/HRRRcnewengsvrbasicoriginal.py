...
ncss = best_ds.subset()
query = ncss.query()
query.lonlat_box(north=55, south=30, east=-60, west=-85).time(datetime.utcnow())
...
for i in range(0,18):
    ...
    ncss = best_ds.subset()
    query = ncss.query()
    query.lonlat_box(north=55, south=30, east=-60, west=-85).time(datetime.utcnow()+dt.timedelta(hours=1*i))
    query.accept('netcdf4')
    query.variables('Temperature_isobaric','u-component_of_wind_isobaric','v-component_of_wind_isobaric','Geopotential_height_isobaric','Composite_reflectivity_entire_atmosphere', 'Convective_available_potential_energy_surface','Convective_inhibition_pressure_difference_layer')
    ...
    ax.set_extent((275, 295, 38, 48), crs = cartopy.crs.PlateCarree())
    # Set a title and show the plot
    ax.set_title('HRRR CAPE (J/kg), Composite Reflectivity (dBZ), and 500mb Geopotential Height (m) at ' + time[0].dt.strftime('%Y-%m-%d %H:%MZ').item())
    plt.savefig(output_dir+'/HRRR_NE/hrrrnewengcaperadar'+time[0].dt.strftime('%Y-%m-%d %H%M').item()+'.png')
    fcst_hr = str(i*1)
