...
for i in range(0,45):
    ...
    query.accept('netcdf4')
    query.variables('Precipitable_water_entire_atmosphere_single_layer','Convective_available_potential_energy_surface','Precipitation_rate_surface','Pressure_reduced_to_MSL_msl','Vertical_velocity_pressure_isobaric','Relative_humidity_isobaric','Temperature_isobaric','u-component_of_wind_isobaric','v-component_of_wind_isobaric','Geopotential_height_isobaric')
    ...
    data = data.rename({
        ...
        'Precipitable_water_entire_atmosphere_single_layer':'pwat'})
    ...
    pwat_c = ax.contourf(x,y,pwat, levels = [.01,.2,.4,.6,.8,1.0,1.2,1.4,1.6,1.8,2.0,2.2,2.4,2.6,2.8,3.0,3.2], cmap = 'winter', alpha = 0.7)
    cbr = fig.colorbar(pwat_c, orientation='vertical', pad = 0.01, aspect = 25,
                        panchor = (0.999,0.5), ax = ax, extendrect=False,
                        ticks=[0.1,0.5,1,1.5,2,2.5,3], shrink = 0.8)
    ...
    jet300 = ax.barbs(xm2[wind_slice,wind_slice], ym2[wind_slice,wind_slice],
                    u_300m[wind_slice,wind_slice], v_300m[wind_slice,wind_slice],
                    length=8, color='b', label='300-hPa Jet Core Winds (kt)')
    #jet500 = ax.barbs(xm[wind_slice,wind_slice], ym[wind_slice,wind_slice],
    #                u_500m[wind_slice,wind_slice], v_500m[wind_slice,wind_slice],
    #                length=8, color='lime', label='500-hPa Jet Core Winds (kt)')
    jet850 = ax.barbs(xm1[wind_slice,wind_slice], ym1[wind_slice,wind_slice],
                    u_850m[wind_slice,wind_slice], v_850m[wind_slice,wind_slice],
                    length=8, color='r', label='850-hPa Jet Core Winds (kt)')
    ...
    msl_contour = ax.contour(x, y, mslpc, colors='lightgrey', levels=range(940,1040,4))
    ...
    ax.set_title('Precipitable Water Composite Valid at ' + time[0].dt.strftime('%Y-%m-%d %H:%MZ').item(), fontsize=36)
    ...
    leg = plt.legend(handles=[jet300, jet850, black_line, red_line], loc=3,
                     title='Legend',framealpha=1)
    leg.set_zorder(100)

    plt.savefig(output_dir+'/synoptic/pwat_composite_'+time[0].dt.strftime('%Y-%m-%d %H%M').item()+'.png')
    fcst_hr = str(i*3)
