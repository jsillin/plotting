...
for i in range(0,41):
    ...
    zH8 = data['height'].metpy.sel(vertical=850 * units.hPa)
    zH8_crs = zH8.metpy.cartopy_crs
    ...
    dx, dy = mpcalc.lat_lon_grid_deltas(lon, lat, initstring=zH8_crs.proj4_init)
    ...
    fig, ax = plt.subplots(1, 1, figsize=(12, 8), subplot_kw={'projection': zH8_crs})

    # Plot RH as filled contours

    rel_hum = data['relative_humidity'].metpy.sel(vertical=850*units.hPa).squeeze()
    rh = ax.contourf(x, y, rel_hum, levels=[70, 80, 90, 100], alpha = 0.7, colors=['#99ff00', '#00ff00', '#00cc00'])

    vertvel = data['omega'].metpy.sel(vertical=850*units.hPa).squeeze()
    ...
    h_contour = ax.contour(x, y, height_contour, colors='k', levels=range(1200, 1550, 30))
    h_contour.clabel(fontsize=8, colors='k', inline=1, inline_spacing=8, fmt='%i', rightside_up=True, use_clabeltext=True)
    t_contour = ax.contour(x, y, temp_contour, colors='xkcd:light red', levels=range(-40, 30, 5), alpha=0.8, linestyles='--')
    ...
    ax.set_title('850 hPa Heights (m), Temperature (\u00B0C), Humidity (%), and Vertical Velocity (Pa/s) at ' + time[0].dt.strftime('%Y-%m-%d %H:%MZ').item())
    plt.savefig(output_dir+'/H8/height_temp_RH_'+time[0].dt.strftime('%Y-%m-%d %H%M').item()+'.png')
    fcst_hr = str(i*3)
