...
for i in range(0,18):
    ...
    ncss = best_ds.subset()
    query = ncss.query()
    query.lonlat_box(north=48, south=41, east=-66, west=-78).time(datetime.utcnow()+dt.timedelta(hours=1*i))
    query.accept('netcdf4')
    ...
    fig, ax = plt.subplots(1, 1, figsize=(30, 30), subplot_kw={'projection': zH5_crs})
    tmp_2m = ax.contourf(x,y,t2m,cmap='RdYlBu_r', alpha = 0.8, levels = range(30,100,5))
    cbr = fig.colorbar(tmp_2m, orientation = 'vertical', pad = 0.01, aspect = 25,
                        panchor = (0.999,0.5), ax = ax, extendrect=False, ticks = range(30,100,5), shrink = 0.80)
    cbr.set_label('2m Temperature (F)', fontsize = 24)

    td_2m = ax.contour(x,y,td2ms,cmap='YlGn',levels=range(30,75,5),linewidths=4)
    td_2m.clabel(fontsize=30,colors='g',inline=1,inline_spacing=4,fmt='%i',rightside_up=True,use_clabeltext=True)
    ...
    h_contour.clabel(fontsize=30, colors='k', inline=1, inline_spacing=4, fmt='%i', rightside_up=True, use_clabeltext=True)
    ...
    ax.barbs(x[wind_slice],y[wind_slice],u_10m[wind_slice,wind_slice],v_10m[wind_slice,wind_slice], length=10)
    ...
    ax.add_feature(cfeature.STATES.with_scale('50m'), linewidths = 4, edgecolor='#000000', zorder=0)
    ax.add_feature(cfeature.LAKES.with_scale('50m'), facecolor=cfeature.COLORS['water'], edgecolor='#c7c783', zorder=0)
    ax.set_extent((284, 293, 41, 47), crs = cartopy.crs.PlateCarree())
    ...
    plt.savefig(output_dir+'/HRRR_NE/hrrr_new_eng_temperatures'+time[0].dt.strftime('%Y-%m-%d %H%M').item()+'.png')
    fcst_hr = str(i*1)
