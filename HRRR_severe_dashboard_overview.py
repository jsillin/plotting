...
for i in range(6,49):
    ...
    ########## PLOTTING #######################################################
    tmp_2m = ax5.contourf(x,y,t2m,cmap='RdYlBu_r', alpha = 0.8, levels = range(-20,100,5),transform=zH5_crs)
    tmp_2m32 = ax5.contour(x,y,t2m,colors='b', alpha = 0.8, levels = [32])
    cbr = fig.colorbar(tmp_2m, orientation = 'vertical', aspect = 80, ax = ax5, pad = 0.01,
    ...
    h_contour.clabel(fontsize=14, colors='lightgray', inline=True, fmt='%i mb', rightside_up=True, use_clabeltext=True)

    ref_levs = range(5,75,1)

    mlc = ax1.contourf(x,y,mllapse,colors=['lightcoral','orangered','orange'],levels=[-10,-8,-7],alpha=0.25)
    mlcc = ax1.contour(x,y,mllapse,colors=['lightcoral','orangered','orange'],levels=[-10,-8,-7],linewidths=1.5)

    tdc = ax1.contour(x,y,td2m,colors='lawngreen',levels=[60],linewidths=1.5)
    tdf = ax1.contourf(x,y,td2m,colors=['lawngreen','green'],levels=[60,100],alpha=0.2)
    cape_summary = ax1.contourf(x,y,cape,levels=[500,1000,1500,2000,2500,3000,3500,4000],alpha=0.5,colors=['plum','violet','orchid','palevioletred','hotpink','deeppink','mediumvioletred','magenta'])

    refc = ax1.contourf(x,y,reflectivity,norm=newnorm,cmap=newcmap,levels=range(5,75,1))
    ...
    capep = ax4.contourf(x, y, cape, levels=[100, 250, 500, 750, 1000, 1250, 1500, 1750, 2000, 2500, 3000], extend='max', alpha = 0.7, cmap='RdPu')#['#0099ff00', '#4066ffb3', '#8066ff8c', '#BF66ff66','#8cff66','#b3ff66','#d9ff66','#ffff66','#ffd966','#ffcc66','#ffb366','#ff8c66','#ff6666','#ff668c','#ff66b3','#ff66d9','#ff66ff'])
    ...
    cb.set_label('SBCAPE (J/kg)', size='large')
    #smap.plot_soundings(fig,ax4,data['temperature'],data['rh'],31,99.5,'regional',cape=True)

    #ax4.barbs(x[ws2],y[ws2],s5u[ws2,ws2],s5v[ws2,ws2], length = 7)
    white = mpatches.Patch(color='#6ec6fd', label='Snow')
    orange = mpatches.Patch(color='orange', label='7-8 C/km')
    pink = mpatches.Patch(color='orangered', label='>8 C/km')
    green = mpatches.Patch(color='lawngreen', label='Td >60F')

    leg = ax1.legend(handles=[pink,orange,green],loc=4,framealpha=1)
    leg.set_zorder(100)

    ax1.barbs(x[wind_slice],y[wind_slice],u_10m[wind_slice,wind_slice],v_10m[wind_slice,wind_slice], length=6)
    ax1.set_title('Reflectivity (dBZ), 700-500mb Lapse Rates (C/km), 10m Winds (kts), and MSLP (mb)',fontsize=14)
    ...
    ## set bounds from center point. 15 lon, 12 lat.

    ax1.set_extent((242, 288, 24, 50))#, crs = zH5_crs)    # Set a title and show the plot
    ax2.set_extent((250, 288, 24, 49))#, crs = zH5_crs)    # Set a title and show the plot
    ax3.set_extent((250, 288, 24, 49))#, crs = zH5_crs)    # Set a title and show the plot
    ax4.set_extent((250, 288, 24, 49))#, crs = zH5_crs)    # Set a title and show the plot
    ax5.set_extent((250, 288, 24, 49))#, crs = zH5_crs)    # Set a title and show the plot
    #fig.canvas.draw()
    fig.tight_layout()
    plt.savefig(output_dir+'/HRRR_ex/CONUS_svr_dash_v4_'+dtfs+'_.png',bbox_inches='tight',pad_inches=0.1)
